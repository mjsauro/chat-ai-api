...
app.post('/chat', async (req: Request, res: Response): Promise<any> => {
	...
	try {
		...
		if (!userResponse.users.length) {
			return res.status(404).json({ error: 'User not found.  Please reigster first.' });
		}
		...
		if (!existingUser.length) {
			return res.status(404).json({ error: 'User not found in database, please reigster first.' });
		}
...
app.post('/get-messages', async (req: Request, res: Response): Promise<any> => {
	...
	try {
		...
		console.log('Error fecthing chat history', error);
